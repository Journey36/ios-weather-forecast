...
## 🤩 `HIG`문서를 통해 `UI` 개선 한 경험이 가장 재미있었음

- 사용자가 느끼는 앱의 완성도는 UI의 사소한 디테일에서 비롯된다고 생각한다. 그래서 주어진 기능 명세서의 기능만 구현하는 것이 계속 마음 불편했다.
- 또, 개발 하다보면 기능 명세서에 명시되지 않은 것을 구현해야할 때가 있는데, 어떻게 해야할지 막막했다.
- 이럴 때 HIG를 보면 해답지를 보는 것처럼 구체적인 가이드를 확인할 수 있었고, 불편했던 구석이 말끔히 해결되는 것이 너무 즐거웠다.

## 🔧 `MVC` 패턴을 준수하며 문제점인 `Massive-ViewController` 개선

- 애플이 UIKit을 사용하는 iOS 앱 개발에 MVC 패턴을 권장한다는 것만 알고, 구체적으로 어떻게 하는 것인지 잘 몰랐다.
- 이 프로젝트를 MVC 패턴으로 구현해 보며 애플의 MVC 패턴을 경험해 보았다.
- 또, MVC 패턴의 문제점인 `Massive-ViewController`를 최소화하도록 개선해보았다.

## 📡 `API 통신`을 담당하는 네트워킹 객체의 `재사용`을 고려한 설계

- `API 통신`은 대부분의 앱에서 사용되는 필수적인 기능이다.
- `URLSession`을 사용하여 API에 데이터를 요청하고 로드하는 절차를 에러 처리를 고려하여 구현했다.
- `OpenWeather`에서 지원하는 여러 API에서 쉽게 재사용할 수 있도록 설계해 보았다.
...
- 🚀 현재 위치의 날씨 정보를 알려주는 앱
    ...
    - [OpenWeather](https://openweathermap.org/) API를 사용하여 현재 위치의 날씨 정보를 표시
- 🗓 진행 기간: 2021.09 ~ 2021.10
    - 캠프 프로젝트 기간(2021.01.18 ~ 24) 1주일 동안 완성하지 못했고, 이후 리팩토링 및 미비한 기능을 완성한 기간
- 📌 사용 언어
    - `Swift`
- 🛠 사용 기술
    - `CoreLocation`, `URLSession`, `Table View`, `Auto Layout`
...
## Human Interface Guidelines 문서를 통한 UI 개선
